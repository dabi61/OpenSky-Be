...
public interface IVoucherRepository
{
    ...
    Task<IEnumerable<Voucher>> GetByTableTypeAsync(TableType tableType); // Lấy voucher theo loại
    ...
    Task<UserVoucher?> SaveVoucherForUserAsync(Guid voucherId, Guid userId); // Khách hàng lưu voucher
    ...
    Task<IEnumerable<UserVoucher>> GetUserSavedVouchersAsync(Guid userId); // Lấy voucher đã lưu của user
    Task<bool> HasUserSavedVoucherAsync(Guid voucherId, Guid userId); // Kiểm tra user đã lưu voucher chưa
